...
function loadGroup(req, res, next) {
  ...
  if (!memberIds(group.id).includes(req.user.id)) return res.status(404).json({ error: 'Group not found' })
  req.group = group
  next()
}
...
function serializeGroup(group) {
  ...
  const expenses = db
    ...
    .map((e) => ({
      ...
      shares: db.prepare('SELECT user_id AS userId, share FROM expense_shares WHERE expense_id = ?').all(e.id),
    }))
...
groupRoutes.post('/', (req, res) => {
  ...
  db.transaction(() => {
    db.prepare(
      `INSERT INTO groups (id, name, ride_id, created_by, created_at)
       VALUES (@id, @name, @ride_id, @created_by, @created_at)`,
    ).run(group)
    const insert = db.prepare('INSERT INTO group_members (group_id, user_id, joined_at) VALUES (?, ?, ?)')
    for (const id of known) insert.run(group.id, id, now())
  })()

  res.status(201).json({ group: serializeGroup(db.prepare('SELECT * FROM groups WHERE id = ?').get(group.id)) })
})

groupRoutes.get('/:id', loadGroup, (req, res) => res.json({ group: serializeGroup(req.group) }))

groupRoutes.post('/:id/members', loadGroup, (req, res) => {
  ...
  res.json({ group: serializeGroup(req.group) })
})

groupRoutes.delete('/:id/members/:userId', loadGroup, (req, res) => {
  ...
  const paid = db.prepare('SELECT COUNT(*) AS n FROM expenses WHERE group_id = ? AND paid_by = ?')
    .get(req.group.id, userId).n
  if (involved > 0 || paid > 0) {
...
function readExpenseInput(req, group) {
...
groupRoutes.post('/:id/expenses', loadGroup, (req, res) => {
  if (req.group.settled_at) return res.status(409).json({ error: 'This group is closed' })
  const { value, error } = readExpenseInput(req, req.group)
  ...
  db.transaction(() => {
    ...
    const insert = db.prepare('INSERT INTO expense_shares (expense_id, user_id, share) VALUES (?,?,?)')
    for (const s of value.shares) insert.run(id, s.userId, s.share)
  })()

  res.status(201).json({ group: serializeGroup(req.group) })
})

/** Only the person who logged an expense may edit it (feedback: expense modification). */
groupRoutes.patch('/:id/expenses/:expenseId', loadGroup, (req, res) => {
  const expense = db.prepare('SELECT * FROM expenses WHERE id = ? AND group_id = ?')
  ...
  db.transaction(() => {
    db.prepare(
      `UPDATE expenses SET description=?, category=?, amount=?, paid_by=?, spent_at=?, updated_at=?
        WHERE id=?`,
    ...
    const insert = db.prepare('INSERT INTO expense_shares (expense_id, user_id, share) VALUES (?,?,?)')
    for (const s of value.shares) insert.run(expense.id, s.userId, s.share)
  })()

  res.json({ group: serializeGroup(req.group) })
})

groupRoutes.delete('/:id/expenses/:expenseId', loadGroup, (req, res) => {
  const expense = db.prepare('SELECT * FROM expenses WHERE id = ? AND group_id = ?')
    .get(req.params.expenseId, req.group.id)
  if (!expense) return res.status(404).json({ error: 'Expense not found' })
  if (expense.created_by !== req.user.id) {
...
groupRoutes.post('/:id/settlements', loadGroup, (req, res) => {
  ...
  const balances = computeBalances(req.group.id)
...
groupRoutes.post('/:id/close', loadGroup, (req, res) => {
  ...
  db.prepare('UPDATE groups SET settled_at = ? WHERE id = ?').run(now(), req.group.id)
  res.json({ group: serializeGroup(db.prepare('SELECT * FROM groups WHERE id = ?').get(req.group.id)) })
})
